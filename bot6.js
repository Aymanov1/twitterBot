...
var smallarray=[];
  T.get('friendships/show', {  target_screen_name: username },  function (err, data, response) {
     for (item in data) {
      for (subItem in data[item]) {
      //follow that target follower 
      if(typeof data[item][subItem].screen_name !== "undefined" && data[item][subItem].followed_by == true && data[item][subItem].following ==false)
       smallarray.push(data[item][subItem].screen_name);
   
          }
     } 
   console.log(uniq(smallarray));
return uniq(smallarray);
    });
}
